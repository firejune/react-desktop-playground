...
class Tab extends Component {
  ...
  render() {
    ...
    if (this.props.firstChild) {
      componentStyle = { ...componentStyle, ...styles.firstChild }
    }

    if (this.props.lastChild) {
      componentStyle = { ...componentStyle, ...styles.lastChild }
    }

    if (nextSelected) {
      componentStyle = { ...componentStyle, ...styles.nextSelected }
    }

    if (this.state.selected && this.state.windowFocused) {
      componentStyle = { ...componentStyle, ...styles.selected }
      if (this.props.firstChild) {
        componentStyle = { ...componentStyle, paddingLeft: '11px' }
      }
      if (this.props.lastChild) {
        componentStyle = { ...componentStyle, borderRightWidth: '1px', paddingRight: '11px' }
      }
      cssStyle = { ...cssStyle, ':active': styles.selectedActive }
    } else if (this.state.selected) {
      componentStyle = { ...componentStyle, ...styles.selectedUnfocused }
      if (this.props.firstChild) {
        componentStyle = { ...componentStyle, paddingLeft: '11px' }
      }
      if (this.props.lastChild) {
